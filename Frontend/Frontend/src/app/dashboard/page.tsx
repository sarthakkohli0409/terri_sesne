...
type RunResult = { mapUrl: string } | null;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (mapRes.status === "fulfilled") {
        setResult({ mapUrl: URL.createObjectURL(new Blob([mapRes.value.data])) });
      ...
      <div className="page">
        ...
        {result && (
          <div className="map-panel" ref={mapRef}>
            ...
            <iframe className="map" src={result.mapUrl} title="Territory map" sandbox="allow-scripts allow-same-origin" />
          </div>
        )}
      </div>
    </>
