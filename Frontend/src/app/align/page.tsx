...
Adjust the sliders on the left if needed, then confirm when ready.`,
  ...
  function renderLeftPanel() {
    ...
    if (currentStep === "segments") {
      // Decile-based segments — D10 highest to D1 lowest
      const DECILE_SEGS = [
        { deciles: "D9 – D10", label: "Segment A", color: "#1D4ED8", defaultCalls: 12 },
        { deciles: "D6 – D8",  label: "Segment B", color: "#15803D", defaultCalls: 8  },
        { deciles: "D3 – D5",  label: "Segment C", color: "#B45309", defaultCalls: 4  },
        { deciles: "D1 – D2",  label: "Segment D", color: "#6B7280", defaultCalls: 0  },
      ];
      return (
        <>
          <GroqBlock />
          <div style={styles.card}>
            ...
            <div style={{ padding: "10px 14px 4px" }}>
              <div style={{ fontSize: 11, color: "#6B7280", marginBottom: 10, lineHeight: 1.5 }}>
                Deciles rank all HCPs by composite score (D10 = top 10%, D1 = bottom 10%).
                Set the call frequency for each segment. Segments marked as Target will be included in territory planning.
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr>
                    {["Segment","Deciles","Calls / year","Target?"].map(h => (
                      <th key={h} style={{ ...styles.schemaTh, textAlign: "left" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {DECILE_SEGS.map((ds, i) => (
                    <tr key={i}>
                      <td style={{ padding: "8px 8px", borderBottom: "1px solid #F0EEE8" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ width: 8, height: 8, borderRadius: "50%", background: ds.color, flexShrink: 0 }} />
                          <input
                            value={segments[i]?.name || ds.label}
                            ...
                            style={{ border: "none", background: "transparent", fontSize: 12,
                              fontFamily: "inherit", color: "#1C1B18", width: 90, outline: "none" }}
                          />
                        </div>
                      </td>
                      <td style={{ padding: "8px 8px", borderBottom: "1px solid #F0EEE8",
                        fontFamily: "monospace", fontSize: 11, color: "#1D4ED8", fontWeight: 500 }}>
                        {ds.deciles}
                      </td>
                      <td style={{ padding: "8px 8px", borderBottom: "1px solid #F0EEE8" }}>
                        <input
                          type="number" min={0} max={52}
                          value={segments[i]?.callFreq || ds.defaultCalls}
                          onChange={(e) => setSegments(prev => {
                            const next = [...prev];
                            if (next[i]) next[i] = { ...next[i], callFreq: e.target.value };
                            return next;
                          })}
                          style={{ width: 60, padding: "4px 6px", border: "1px solid #E2E0D8",
                            borderRadius: 4, fontFamily: "monospace", fontSize: 12,
                            textAlign: "center", outline: "none" }}
                        />
                      </td>
                      <td style={{ padding: "8px 8px", borderBottom: "1px solid #F0EEE8", textAlign: "center" }}>
                        <input
                          type="checkbox"
                          checked={segments[i]?.target ?? (ds.defaultCalls > 0)}
                          onChange={(e) => setSegments(prev => {
                            const next = [...prev];
                            if (next[i]) next[i] = { ...next[i], target: e.target.checked };
                            return next;
                          })}
                        />
                      </td>
