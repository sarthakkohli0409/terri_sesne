...
export default function AlignPage() {
  ...
  const [segments, setSegments] = useState<AlignSegRow[]>([
    { name: "Very High", startDecile: null, endDecile: null, callsPerHcp: "", target: null },
    { name: "High",      startDecile: null, endDecile: null, callsPerHcp: "", target: null },
    { name: "Medium",    startDecile: null, endDecile: null, callsPerHcp: "", target: null },
    { name: "Low",       startDecile: null, endDecile: null, callsPerHcp: "", target: null },
    { name: "Very Low",  startDecile: null, endDecile: null, callsPerHcp: "", target: null },
  ]);
...
` +
    ...
    if (FLOW[nextIdx - 1]?.qid === "Q-004") {
      // Don't release isWaiting yet — wait for analyze to finish
      addBotMessage(
        "Calculating your optimal territory count based on segment call frequencies...",
        "Q-005"
      );
      await runAnalyze();
      // Now update the Q-005 message with the computed K
      addBotMessage(
        "Based on your segment configuration, I've calculated the optimal number of territories. " +
        "Review and adjust the parameters on the left, then confirm when ready.",
        "Q-005",
        ["Parameters set — generate map"]
      );
      setIsWaiting(false);
      return;
    }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      try {
        ...
        addBotMessage("Map is ready! Excel download failed — use the Download Excel button in the left panel to retry.", "Q-FIN");
      }
                      ...
                      <tr key={idx} style={{ borderBottom: "1px solid #F0EEE8" }}>
                        ...
                        <td style={{ padding: "8px 10px", textAlign: "center" }}>
                          <select style={{ ...styles.segSelectSm, width: 60 }}
                            ...
                            onChange={e => updateSeg(idx, "target", e.target.value === "" ? null : e.target.value === "yes")}>
                            <option value="">—</option>
            ...
            <div style={{ padding: "14px 16px" }}>
              {analyzing && (
                <div style={{ background: "#EFF4FF", border: "1px solid #C3D4FD", borderRadius: 6,
                  ...
                  <div style={{ width: 12, height: 12, borderRadius: "50%", border: "2px solid #1D4ED8",
