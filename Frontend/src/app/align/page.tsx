...
Adjust the sliders on the left if needed, then confirm when ready.`,
  ...
  // ── Run /analyze after segments to compute K ─────────────────────────────
  async function runAnalyze(): Promise<number | null> {
    if (!file || weights.length === 0) return null;
    setAnalyzing(true);
    try {
      const cfg: Record<string,number> = {};
      ...
      const activeSegs = segments
        .filter(s => s.target === true && s.startDecile !== null && s.endDecile !== null)
        .map(s => ({
          segment: s.name,
      ...
      fd.append("file", file);
      fd.append("column_config", JSON.stringify(cfg));
      fd.append("segment_config", JSON.stringify(activeSegs));
      const res = await axios.post(`${API}/analyze`, fd, { timeout: 120000 });
      let computedK: number | null = null;
      if (res.data && !res.data.error) {
        setAnalyzeResult(res.data);
        computedK = Math.ceil(res.data.calls_required / (workingDays * callsPerDay));
        if (computedK > 0) setNumClusters(computedK);
      }
      return computedK;
    } catch { /* non-blocking */ return null; }
