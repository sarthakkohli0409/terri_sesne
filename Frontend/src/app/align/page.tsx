...
type SizingMethod = "call_based" | "workload" | "revenue";
...
const FLOW: FlowStep[] = [
  {
    ...
    question: "Hi! I'm your TerriSense Alignment Assistant.\n\nI guide you through a series of questions — about your data, how you want to weight your metrics, how to segment your customers, and how many territories you need — and at the end you get a fully optimised territory map. Ready to get started?",
  ...
  },
  {
    ...
    question: "Now let's define your customer segments. Segments are based on deciles — D10 is your highest-value HCPs, D1 is the lowest.\n\nReview and adjust the segment definitions and call frequencies on the left panel, then confirm when ready.",
    chips: ["Segments configured — continue"],
  ...
  },
  {
    ...
    question: "Based on your segment configuration, I've calculated your optimal territory count. Review the parameters on the left — you can adjust K, tolerance, and hard floor — then confirm when ready.",
    chips: ["Parameters set — generate map"],
...
// ── Align segment type (mirrors dashboard SegmentRow) ───────────────────────
type AlignSegRow = {
  name: string;
  startDecile: number | null;
  endDecile: number | null;
  callsPerHcp: number | string;
  target: boolean | null;
};


// ── Stable map iframe — memoized so map never re-renders on state change ─────
const MapFrame = React.memo(function MapFrame({ html }: { html: string }) {
  return (
    <iframe
      srcDoc={html}
      title="Territory map"
...
export default function AlignPage() {
  ...
  const [segments, setSegments] = useState<AlignSegRow[]>([
    { name: "Very High", startDecile: null, endDecile: null, callsPerHcp: "", target: true },
    { name: "High",      startDecile: null, endDecile: null, callsPerHcp: "", target: true },
    { name: "Medium",    startDecile: null, endDecile: null, callsPerHcp: "", target: true },
    { name: "Low",       startDecile: null, endDecile: null, callsPerHcp: "", target: true },
    { name: "Very Low",  startDecile: null, endDecile: null, callsPerHcp: "", target: false },
  ]);
  // Note: segments start with null deciles — user activates each one via checkbox.
  // toggleSegment() and updateSeg() cascade D10 to the highest active segment automatically.
  const [numClusters, setNumClusters] = useState(50);
  const [preferState, setPreferState] = useState(false);
  const [sizingMethod, setSizingMethod] = useState<SizingMethod>("call_based");
  const [avgCallDuration, setAvgCallDuration] = useState(30); // minutes
  const [travelPct, setTravelPct] = useState(25); // % of day spent travelling
  const [avgRevenuePerRep, setAvgRevenuePerRep] = useState(500000); // $ per rep per year
  const [totalRevenuePotential, setTotalRevenuePotential] = useState(0); // computed or entered
  ...
  // Results
  const [analyzeResult, setAnalyzeResult] = useState<{calls_required:number}|null>(null);
  ...
  useEffect(() => {
    // Ping backend on load to wake Render free tier before user generates
  ...
  async function showGroqAndQuestion(step: FlowStep, idx: number) {
    // Q-001 (mode) is handled silently — no chat message shown, auto-continues
    if (step.qid === "Q-001") {
  ...
  async function handleReply(text: string) {
    ...
    // ── Excel download chips ──────────────────────────────────────────────
    if (text.trim() === "Yes, download Excel") {
      setIsWaiting(true);
      await handleDownloadExcel();
      setIsWaiting(false);
      return;
    }
    if (text.trim() === "No thanks") {
      addBotMessage("No problem! Your map is ready on the left.", currentFlowStep?.qid || "Q-FIN");
      setIsWaiting(false);
      return;
    }

    // ── Resume flow after free-form Q&A ─────────────────────────────────
    if (text.trim() === "Yes, let's continue") {
      const currentFlowStep2 = FLOW[currentFlowIdx];
      if (currentFlowStep2) {
        await showGroqAndQuestion(currentFlowStep2, currentFlowIdx);
      }
      setIsWaiting(false);
      return;
    }

    if (text.trim() === "I have another question") {
      addBotMessage("Of course — go ahead, ask anything.", currentFlowStep?.qid || "Q-FIN");
      setIsWaiting(false);
      return;
    }
    ...
    // ── Tell me more first — explain the process ───────────────────────────
    if (text.trim() === "Tell me more first") {
      addBotMessage(
        "Sure! Here's how TerriSense works:\n\n" +
        "1. You upload your HCP data — a file with customer IDs, ZIP codes, and sales metrics like Rx volume or call activity.\n\n" +
        "2. You set metric weights — deciding which metrics matter most for defining territory value.\n\n" +
        "3. You define customer segments — grouping HCPs by decile (D10 = highest value, D1 = lowest) with different call frequencies per group.\n\n" +
        "4. The system calculates the optimal number of territories based on total call workload and rep capacity.\n\n" +
        "5. It generates a territory map — geographically balanced, index-optimised, ready to present.\n\nShall we get started?",
        "Q-000",
        ["Yes, let's go"]
      );
      setIsWaiting(false);
...
` +
    ...
    // ── GATE: Q-002 cannot advance without a confirmed file upload ────────
    if (currentFlowStep.qid === "Q-002" && !colsConfirmed) {
      addBotMessage(
        "I still need your data file before we can move forward. " +
        "Please upload it using the panel on the left — I'll detect your metric columns automatically.",
        "Q-002"
      );
      setIsWaiting(false);
      return;
    ...
    // ── GATE: Q-003 weights must sum to 100% ─────────────────────────────
    if (currentFlowStep.qid === "Q-003") {
      const total = weights.reduce((s, r) => s + Number(r.weight || 0), 0);
      if (total !== 100) {
        addBotMessage(
          "The metric weights currently add up to " + total + "% — they need to equal exactly 100% before we continue. " +
          "Adjust the sliders on the left and try again.",
          "Q-003"
        );
        setIsWaiting(false);
        return;
    ...
    if (nextIdx >= FLOW.length) {
      // End of flow — validate file one last time before generating
      if (!file) {
        addBotMessage(
          "It looks like no file was uploaded. Please go back and upload your data file first.",
          "Q-FIN"
        );
        setCurrentFlowIdx(currentFlowIdx);
        setIsWaiting(false);
        return;
      }
      setTimeout(() => {
    ...
    // After segments step → run /analyze to compute K, then show Q-005
    if (FLOW[nextIdx - 1]?.qid === "Q-004") {
      await runAnalyze();
      // Q-005 question is already shown by showGroqAndQuestion above
      // Just update it with K value if computed
      setIsWaiting(false);
      return;
    }
  ...
  async function sendFreeText() {
    ...
    // Check if this matches a chip option for current step
    if (currentFlowStep?.chips?.includes(text)) {
      handleReply(text);
      return;
    }
    ...
    // Free-form question — send to Groq with full conversation history
    setIsWaiting(true);
    addUserMessage(text, currentFlowStep?.qid || "Q-FIN");

    // Add to conversation history
    const newHistory: ConvMessage[] = [...convHistory, { role: "user" as const, content: text }];
    setConvHistory(newHistory);

    const response = await groqChat(text, convHistory, currentStep);

    // Add to history
    setConvHistory(prev => [...prev, { role: "assistant" as const, content: response }]);

    addBotMessage(response, currentFlowStep?.qid || "Q-FIN", ["Yes, let's continue", "I have another question"]);
    setIsWaiting(false);
  }


  // ── Groq conversational chat ──────────────────────────────────────────────
  async function groqChat(userMsg: string, history: ConvMessage[], currentStepId: string): Promise<string> {
    const systemPrompt = `You are TerriSense, a pharmaceutical territory alignment assistant. 
You are in the middle of guiding a user through a territory alignment process.
Current step: ${currentStepId}.

The full process has these steps in order:
1. Data upload — user uploads HCP data file with ZIP codes and metrics
2. Metric weights — user assigns percentage weights to metric columns (must sum to 100%)
3. Customer segments — user defines decile-based segments (D10=top, D1=bottom) with call frequencies
4. Territory sizing — system calculates optimal K territories based on call frequencies
5. Map generation — system generates the territory map

Answer the user's question fully and naturally. Use the conversation history for context.
After answering, always end with: "Ready to get back to the alignment?"
Keep answers concise — 2-4 sentences max unless a detailed explanation is needed.`;

    if (!GROQ_KEY) {
      return "I'd be happy to answer that, but I need a Groq API key to respond to free-form questions. Ready to get back to the alignment?";
    }
    ...
    try {
      ...
      const d = await res.json();
      return d.choices?.[0]?.message?.content?.trim() || "Good question — Ready to get back to the alignment?";
    } catch {
      return "I couldn't process that right now. Ready to get back to the alignment?";
    }
  }
...
` +
      ...
      // ── Success — set up weights and ask for confirmation (fires ONCE) ──
      setDetectedCols(metricCols);
      const eq = Math.floor(100 / metricCols.length);
      const newWeights = metricCols.map((c, i) => ({
        id: i + 1,
        column: c,
      ...
      addBotMessage(
        ...
        metricCols.map(function(c) { return "- " + c; }).join("\n") +
...
Adjust the sliders on the left if needed, then confirm when ready.`,
  ...
  async function handleDownloadExcel() {
    ...
    fd.append("prefer_state", String(preferState));
    try {
      const res = await axios.post(`${API}/optimize_excel`, fd, { responseType: "blob", timeout: 300000 });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(new Blob([res.data]));
      a.download = "territory_analysis.xlsx";
      a.click(); a.remove();
    } catch {
      setGenError("Excel download failed — please try again.");
    } finally { setDownloadingExcel(false); }
  }

  // ── Compute K locally from segment config — no API call needed ────────────
  async function runAnalyze(): Promise<number | null> {
    setAnalyzing(true);
    try {
      // Count total HCPs per decile from the file via /analyze
      // BUT also compute K locally as a fast estimate from segment call frequencies
      // Local estimate: use HCP count proportional to decile ranges
      const activeSegs = segments.filter(s =>
        s.target === true && s.startDecile !== null && s.endDecile !== null
      );
      if (activeSegs.length === 0) return null;

      // Try API first for accurate HCP counts
      if (file && weights.length > 0) {
        try {
          const cfg: Record<string,number> = {};
          weights.filter(w => w.column.trim()).forEach(w => { cfg[w.column.trim()] = Number(w.weight); });
          const segCfg = activeSegs.map(s => ({
            segment: s.name,
            start_decile: Math.max(s.startDecile!, s.endDecile!),
            end_decile: Math.min(s.startDecile!, s.endDecile!),
            calls_per_hcp: Number(s.callsPerHcp) || 0,
            target: true,
          }));
          const fd = new FormData();
          fd.append("file", file);
          ...
          if (res.data && !res.data.error && res.data.calls_required > 0) {
            setAnalyzeResult(res.data);
            const k = Math.max(1, Math.ceil(res.data.calls_required / (workingDays * callsPerDay)));
            setNumClusters(k);
            return k;
          }
      ...
      // Local fallback: estimate K from decile ranges × assumed HCPs per decile
      const totalHCPs = filePreview ? filePreview.rows.length * 10 : 1000;
      let totalCalls = 0;
      activeSegs.forEach(s => {
        const lo = Math.min(s.startDecile!, s.endDecile!);
        const hi = Math.max(s.startDecile!, s.endDecile!);
        const decileCount = hi - lo + 1;
        const hcpsInRange = Math.round((decileCount / 11) * totalHCPs);
        totalCalls += hcpsInRange * (Number(s.callsPerHcp) || 0);
      });

      let k = 1;
      if (sizingMethod === "call_based") {
        k = Math.max(1, Math.ceil(totalCalls / (workingDays * callsPerDay)));
      } else if (sizingMethod === "workload") {
        // Effective calls per day reduced by travel time
        const effectiveCallsPerDay = callsPerDay * (1 - travelPct / 100) * (1 - avgCallDuration / (8 * 60));
        k = Math.max(1, Math.ceil(totalCalls / (workingDays * Math.max(0.5, effectiveCallsPerDay))));
      } else if (sizingMethod === "revenue") {
        // Revenue-based: divide total potential by target per rep
        const pot = totalRevenuePotential > 0 ? totalRevenuePotential : totalCalls * 100;
        k = Math.max(1, Math.ceil(pot / avgRevenuePerRep));
      }
      setNumClusters(k);
      return k;
  ...
  function renderLeftPanel() {
            ...
            <label style={{ ...styles.uploadZone, ...(file ? styles.uploadZoneDone : {}) }}>
              ...
              {detecting ? (
                ...
                <>
                  ...
                  <div style={{ fontSize: 11, color: "#B45309", marginTop: 3 }}>Check the chat for action needed</div>
                </>
              ) : (
                <>
                  <div style={{ fontSize: 20, marginBottom: 6 }}>&#128196;</div>
                  <div style={{ fontSize: 13, color: "#6B7280" }}>Drop file here or click to browse</div>
                  <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>Accepts .xlsx or .csv — max 50 MB</div>
                </>
              )}
            </label>
          </div>
    ...
    if (currentStep === "segments") {
      ...
      const SEG_COLORS = ["#1D4ED8","#15803D","#B45309","#7C3AED","#6B7280"];

      // ── Core logic ─────────────────────────────────────────────────────
      // "Active" = segment is enabled (user toggled it on)
      // The highest active segment always gets startDecile = D10 automatically
      // Each segment's endDecile must be < startDecile of next active segment below it
      // startDecile of each segment = (endDecile of segment above it) - 1

      const toggleSegment = (idx: number) => {
        setSegments((prev: AlignSegRow[]) => {
          const next = [...prev.map(r => ({...r}))];
          const isActive = next[idx].startDecile !== null || next[idx].endDecile !== null || next[idx].callsPerHcp !== "";
          if (isActive) {
            // Deactivate: clear this segment
            next[idx] = { ...next[idx], startDecile: null, endDecile: null, callsPerHcp: "" };
          } else {
            // Activate: will be wired up by cascade below
            next[idx] = { ...next[idx], startDecile: null, endDecile: null, callsPerHcp: "" };
          }
          return cascadeDeciles(next);
        });
      };

      const cascadeDeciles = (segs: AlignSegRow[]): AlignSegRow[] => {
        const next = segs.map(r => ({...r}));
        // Find first active segment (has endDecile set or is the top one being activated)
        // Rule: highest active segment starts at D10
        let prevEnd: number | null = null;
        for (let i = 0; i < next.length; i++) {
          const isActive = next[i].endDecile !== null ||
            (next[i].startDecile !== null) ||
            (i === 0 && next[i].callsPerHcp !== "");
          if (!isActive) {
            next[i] = { ...next[i], startDecile: null };
            continue;
          }
          if (prevEnd === null) {
            // This is the highest active segment — always starts at D10
            next[i] = { ...next[i], startDecile: 10 };
          } else {
            next[i] = { ...next[i], startDecile: Math.max(0, prevEnd - 1) };
          }
          prevEnd = next[i].endDecile;
        }
        return next;
      };

      const updateSeg = (idx: number, field: string, val: unknown) => {
        setSegments((prev: AlignSegRow[]) => {
          const next = prev.map((r: AlignSegRow, i: number) =>
            i === idx ? { ...r, [field]: val } : {...r}
          );
          // After any change, cascade start deciles downward
          let prevEnd: number | null = null;
          for (let i = 0; i < next.length; i++) {
            const active = next[i].endDecile !== null || next[i].startDecile !== null || next[i].callsPerHcp !== "";
            if (!active) { next[i].startDecile = null; continue; }
            if (prevEnd === null) {
              next[i].startDecile = 10; // highest active always = D10
            } else {
              next[i].startDecile = Math.max(0, prevEnd - 1);
            }
            prevEnd = next[i].endDecile;
          }
          return next;
      ...
      const endDecileOptions = (idx: number): number[] => {
        ...
        if (row.startDecile === null) return [];
        const sd = row.startDecile;
        // Find start decile of next active segment to constrain upper bound
        const nextActiveStart = (() => {
          for (let i = idx + 1; i < segments.length; i++) {
            if (segments[i].startDecile !== null) return segments[i].startDecile!;
          }
          return -1;
        })();
        // Highest active segment: end can equal start (single decile allowed)
        // All others: end must be < start (at least one decile range)
        const isHighest = segments.slice(0, idx).every(s => s.startDecile === null && s.endDecile === null && s.callsPerHcp === "");
        return DECILES.filter(d => {
          if (d > sd) return false;
          if (!isHighest && d >= sd) return false;
          if (nextActiveStart !== -1 && d <= nextActiveStart) return false;
          return true;
        });
      };
      ...
      return (
        <>
          <GroqBlock />
          <div style={styles.card}>
            <div style={styles.cardHead}>Segment definitions — decile based</div>
            <div style={{ padding: "10px 0 4px" }}>
              <div style={{ fontSize: 11, color: "#6B7280", padding: "0 14px 10px", lineHeight: 1.5 }}>
                Toggle segments on or off. The highest active segment always starts at D10.
                Each end decile cascades the start of the segment below it automatically.
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr>
                    {["","Segment","Start","End","Calls / HCP / yr","Target?"].map(h => (
                      <th key={h} style={{ ...styles.schemaTh, padding: "6px 8px", textAlign: "left" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {segments.map((row: AlignSegRow, idx: number) => {
                    const isActive = row.startDecile !== null || row.endDecile !== null || row.callsPerHcp !== "";
                    const isHighest = isActive && segments.slice(0, idx).every(
                      s => s.startDecile === null && s.endDecile === null && s.callsPerHcp === ""
                    );
                    const endOpts = endDecileOptions(idx);
                    const isVeryLow = idx === segments.length - 1;
                    const autoEndD0 = isActive && isVeryLow && row.startDecile === 0;
                    const rowStyle: React.CSSProperties = {
                      borderBottom: "1px solid #F0EEE8",
                      opacity: isActive ? 1 : 0.4,
                      background: isActive ? "transparent" : "#FAFAF8",
                      ...
                      <tr key={idx} style={rowStyle}>
                        ...
                        <td style={{ padding: "8px 8px", width: 28 }}>
                          <input type="checkbox"
                            ...
                            style={{ accentColor: SEG_COLORS[idx], cursor: "pointer" }} />
                        </td>
                        {/* Segment name + color dot */}
                        <td style={{ padding: "8px 6px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <div style={{ width: 8, height: 8, borderRadius: "50%", flexShrink: 0,
                              background: SEG_COLORS[idx] }} />
                            <span style={{ fontSize: 12, fontWeight: 500,
                              color: isActive ? "#1C1B18" : "#9CA3AF" }}>
                              {row.name}
                            </span>
                            {isHighest && (
                              <span style={{ fontSize: 9, background: "#EFF4FF", color: "#1D4ED8",
                        ...
                        {/* Start decile — auto D10 for highest, auto-derived for rest */}
                        <td style={{ padding: "8px 6px" }}>
                          <span style={{ fontFamily: "monospace", fontSize: 11,
                            color: isActive ? "#1D4ED8" : "#9CA3AF",
                            fontWeight: isHighest ? 600 : 400 }}>
                            {row.startDecile !== null ? "D" + row.startDecile : "—"}
                            {isHighest && <span style={{ fontSize: 9, color: "#6B7280" }}> auto</span>}
                          </span>
                        </td>
                        {/* End decile — user selects */}
                        <td style={{ padding: "8px 6px" }}>
                          {!isActive ? (
                            <span style={{ fontFamily: "monospace", fontSize: 11, color: "#9CA3AF" }}>—</span>
                          ) : autoEndD0 ? (
                            <span style={{ fontFamily: "monospace", fontSize: 11, color: "#1D4ED8" }}>D0</span>
                          ) : (
                            <select style={{ ...styles.segSelectSm }}
                              disabled={row.startDecile === null}
                              value={row.endDecile !== null ? row.endDecile : ""}
                              onChange={e => updateSeg(idx, "endDecile",
                                e.target.value === "" ? null : Number(e.target.value))}>
                              <option value="">—</option>
                              {endOpts.map(d => <option key={d} value={d}>D{d}</option>)}
                            </select>
                          )}
                        </td>
                        {/* Calls per HCP */}
                        <td style={{ padding: "8px 6px" }}>
                          <input type="number" min={0} max={52}
                            disabled={!isActive}
                            value={row.callsPerHcp}
                            placeholder="0"
                            onChange={e => updateSeg(idx, "callsPerHcp", e.target.value)}
                            style={{ width: 56, padding: "4px 6px", border: "1px solid #E2E0D8",
                              borderRadius: 4, fontFamily: "monospace", fontSize: 12,
                              textAlign: "center", outline: "none",
                              background: !isActive ? "#F5F4F1" : "#fff" }} />
                        </td>
                        {/* Target */}
                        <td style={{ padding: "8px 6px", textAlign: "center" }}>
                          <select style={{ ...styles.segSelectSm, width: 54 }}
                            disabled={!isActive}
                            value={row.target === false ? "no" : "yes"}
                            onChange={e => updateSeg(idx, "target", e.target.value === "yes")}>
    ...
    if (currentStep === "sizing") {
      const methodLabels: Record<string,string> = {
        ...
        workload: "Workload-adjusted",
        revenue: "Revenue potential",
      };
      const methodDesc: Record<string,string> = {
        call_based: "K = total annual calls ÷ rep capacity. Standard approach — driven by call frequency and HCP count.",
        workload: "Adjusts rep capacity for travel time and call duration. Better for geographically sparse territories.",
        revenue: "K = total revenue potential ÷ target revenue per rep. Used when revenue data is available.",
      };
      return (
        <>
          <GroqBlock />
          ...
          <div style={styles.card}>
            <div style={styles.cardHead}>Sizing method</div>
            <div style={{ padding: "12px 14px" }}>
              {(["call_based","workload","revenue"] as const).map(m => (
                <label key={m} style={{ display: "flex", alignItems: "flex-start", gap: 10,
                  marginBottom: 12, cursor: "pointer" }}>
                  <input type="radio" name="sizingMethod" value={m}
                    checked={sizingMethod === m}
                    onChange={() => { setSizingMethod(m); setAnalyzeResult(null); }}
                    style={{ marginTop: 3, accentColor: "#1D4ED8", flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: "#1C1B18" }}>{methodLabels[m]}</div>
                    <div style={{ fontSize: 11, color: "#6B7280", marginTop: 2, lineHeight: 1.5 }}>{methodDesc[m]}</div>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div style={styles.statsGrid}>
            <div style={styles.statCard}>
              <div style={styles.statLabel}>Territories (K)</div>
              <div style={styles.statVal}>
                {analyzing
                  ? <span style={{ fontSize: 12, color: "#1D4ED8" }}>Calculating…</span>
                  : analyzeResult
                    ? numClusters
                    : <span style={{ fontSize: 12, color: "#9CA3AF" }}>—</span>
                }
              </div>
              <div style={styles.statSub}>field reps</div>
            </div>
            {[
          ...
          <div style={styles.card}>
            ...
            <div style={{ padding: "14px 16px" }}>
              {analyzing && (
                <div style={{ background: "#EFF4FF", border: "1px solid #C3D4FD", borderRadius: 6,
                  padding: "12px 14px", marginBottom: 12, fontSize: 12, color: "#1D4ED8",
                  display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid #1D4ED8",
                    borderTopColor: "transparent", animation: "spin 0.7s linear infinite", flexShrink: 0 }} />
                  Calculating optimal K from your segment call frequencies…
                </div>
              )}
              {!analyzing && analyzeResult && (
                <div style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 6,
                  padding: "8px 12px", marginBottom: 12, fontSize: 12, color: "#15803D" }}>
                  ✓ {analyzeResult.calls_required.toLocaleString()} total calls/year ÷ ({workingDays}d × {callsPerDay} calls/day) = <strong>{numClusters} territories</strong>
                </div>
              )}
              {!analyzing && (
                <FormRow label="Number of territories (K)" required>
                  <input
                    type="number" min={2} max={500} value={analyzeResult ? numClusters : ""}
                    placeholder={analyzing ? "Calculating…" : "Waiting for segment config…"}
                    onChange={(e) => setNumClusters(+e.target.value)}
                    style={{ ...styles.formInput, background: !analyzeResult ? "#F5F4F1" : "#fff" }}
                  />
                </FormRow>
              )}
              ...
              {sizingMethod === "workload" && (
                <>
                  <div style={{ height: 1, background: "#E2E0D8", margin: "12px 0" }} />
                  <div style={{ fontSize: 11, fontWeight: 500, color: "#6B7280",
                    fontFamily: "monospace", marginBottom: 10, textTransform: "uppercase" }}>
                    Workload parameters
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <FormRow label="Avg call duration (min)">
                      <input type="number" min={5} max={120} value={avgCallDuration}
                        onChange={e => { setAvgCallDuration(+e.target.value); setAnalyzeResult(null); }}
                        style={styles.formInput} />
                    </FormRow>
                    <FormRow label="Travel time (% of day)">
                      <input type="number" min={0} max={80} value={travelPct}
                        onChange={e => { setTravelPct(+e.target.value); setAnalyzeResult(null); }}
                        style={styles.formInput} />
                    </FormRow>
                  </div>
                </>
              )}
              {sizingMethod === "revenue" && (
                <>
                  <div style={{ height: 1, background: "#E2E0D8", margin: "12px 0" }} />
                  <div style={{ fontSize: 11, fontWeight: 500, color: "#6B7280",
                    fontFamily: "monospace", marginBottom: 10, textTransform: "uppercase" }}>
                  ...
                  <FormRow label="Total revenue potential ($)">
                    <input type="number" min={0} value={totalRevenuePotential}
                      onChange={e => { setTotalRevenuePotential(+e.target.value); setAnalyzeResult(null); }}
                      style={styles.formInput} placeholder="e.g. 20000000" />
                  </FormRow>
                  <FormRow label="Target revenue per rep ($/yr)">
                    <input type="number" min={0} value={avgRevenuePerRep}
                      onChange={e => { setAvgRevenuePerRep(+e.target.value); setAnalyzeResult(null); }}
                      style={styles.formInput} placeholder="e.g. 500000" />
                  </FormRow>
                </>
              )}
              ...
              {!analyzing && (
                <button
                  onClick={() => { setAnalyzeResult(null); runAnalyze(); }}
                  style={{ marginTop: 10, width: "100%", padding: "7px 12px",
                    background: "#EFF4FF", border: "1px solid #C3D4FD", borderRadius: 6,
              ...
              <div style={{ marginTop: 14, padding: "12px 14px", background: "#FAFAF8",
                border: "1px solid #E2E0D8", borderRadius: 8 }}>
                <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer" }}>
                  <input type="checkbox" checked={preferState}
                    onChange={e => setPreferState(e.target.checked)}
                    style={{ marginTop: 2, accentColor: "#1D4ED8", flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: "#1C1B18" }}>
                      Prefer state boundaries
                    </div>
                    <div style={{ fontSize: 11, color: "#6B7280", marginTop: 3, lineHeight: 1.5 }}>
                      Territories will try to stay within state lines. Cross-state assignment
                      is allowed when needed to keep the index within a soft range (±25%).
                    </div>
                  </div>
                </label>
              </div>
      ...
      <>
        ...
        {result && (
          <div ref={mapRef} style={styles.mapPanel}>
            ...
            <MapFrame html={result.mapHtml} />
          </div>
        )}
        {/* Retry Excel button — always visible once map is ready */}
        {result && !generating && (
          <div style={{ padding: "0 0 16px" }}>
            <button
              onClick={handleDownloadExcel}
              disabled={downloadingExcel}
              style={{ width: "100%", padding: "10px 14px", background: downloadingExcel ? "#F5F4F1" : "#1D4ED8",
                color: downloadingExcel ? "#9CA3AF" : "#fff", border: "none", borderRadius: 8,
                fontSize: 13, fontWeight: 500, cursor: downloadingExcel ? "not-allowed" : "pointer",
                fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              {downloadingExcel
                ? <><div style={{ width: 12, height: 12, borderRadius: "50%", border: "2px solid #9CA3AF", borderTopColor: "transparent", animation: "spin 0.7s linear infinite" }} /> Building Excel report...</>
      ...
      <div style={styles.main}>
        ...
        <div style={styles.leftPanel}>
          <div style={styles.panelHeader}>
            <span style={styles.panelTag}>Territory Workspace</span>
              ...
              <div key={msg.id}>
                {msg.role === "assistant" ? (
                  <div style={styles.qMsg}>
                    ...
                    {msg.chips && msg.chips.length > 0 && !msg.chipsDisabled && (
                      <div style={styles.chips}>
                        {msg.chips.map((c) => (
                          <button key={c} style={styles.chip}
                            onClick={() => {
                              setMessages(prev => prev.map(m =>
                                m.id === msg.id ? { ...m, chipsDisabled: true } : m
                              ));
                              handleReply(c);
                            }}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  segSelectSm: { width: 72, padding: "4px 6px", border: "1px solid #E2E0D8", borderRadius: 4, fontSize: 11, fontFamily: "inherit", color: "#1C1B18", background: "#fff", outline: "none" },
