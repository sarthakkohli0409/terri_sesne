...
Adjust the sliders on the left if needed, then confirm when ready.`,
  ...
  async function runAnalyze(): Promise<number | null> {
    ...
    try {
      ...
      // Each decile ≈ 10% of HCPs. Use total HCP count from file preview if available.
      const totalHCPs = filePreview ? filePreview.rows.length * 10 : 1000; // rough estimate
      let totalCalls = 0;
      ...
      const k = Math.max(1, Math.ceil(totalCalls / (workingDays * callsPerDay)));
      setNumClusters(k);
      return k;
    ...
    if (currentStep === "segments") {
      ...
      const updateSeg = (idx: number, field: string, val: unknown) => {
        setSegments((prev: AlignSegRow[]) => {
          const next = prev.map((r: AlignSegRow, i: number) => i === idx ? { ...r, [field]: val } : r);
          // Re-derive start deciles for rows below
          for (let i = 1; i < next.length; i++) {
            const above = next[i-1];
            if (above.endDecile !== null) {
              next[i] = { ...next[i], startDecile: Math.max(0, above.endDecile - 1) };
            } else {
          ...
          <div style={styles.card}>
            ...
            <div style={{ padding: "10px 0 4px" }}>
              <div style={{ fontSize: 11, color: "#6B7280", padding: "0 14px 10px", lineHeight: 1.5 }}>
                HCPs are ranked D10 (top 10% by composite score) to D1 (bottom 10%).
                Set decile ranges, call frequency, and whether each segment is targeted.
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr>
                    {["Segment","Start decile","End decile","Calls / HCP / yr","Target?"].map(h => (
                      <th key={h} style={{ ...styles.schemaTh, padding: "6px 10px", textAlign: "left" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {segments.map((row: AlignSegRow, idx: number) => {
                    ...
                    const isLocked = row.startDecile === null && idx > 0;
                    // Very Low: if startDecile is D0, end is also D0 automatically
                    const isVeryLow = idx === segments.length - 1;
                    const autoEndD0 = isVeryLow && row.startDecile === 0;

                    return (
                      <tr key={idx} style={{ borderBottom: "1px solid #F0EEE8" }}>
                        <td style={{ padding: "8px 10px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <div style={{ width: 8, height: 8, borderRadius: "50%", flexShrink: 0,
                              background: ["#1D4ED8","#15803D","#B45309","#7C3AED","#6B7280"][idx] }} />
                            <span style={{ fontSize: 12, fontWeight: 500 }}>{SEGMENT_NAMES[idx]}</span>
                          </div>
                        </td>
                        <td style={{ padding: "8px 10px" }}>
                          {idx === 0 ? (
                            <select style={styles.segSelectSm}
                              value={row.startDecile !== null ? row.startDecile : ""}
                              onChange={e => updateSeg(0, "startDecile", e.target.value === "" ? null : Number(e.target.value))}>
                              <option value="">—</option>
                              {DECILES.map(d => <option key={d} value={d}>D{d}</option>)}
                            </select>
                          ) : (
                            <span style={{ fontFamily: "monospace", fontSize: 11, color: "#1D4ED8" }}>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          {autoEndD0 ? (
                            ...
                            <select style={styles.segSelectSm}
                              disabled={isLocked || row.startDecile === null}
                              value={row.endDecile !== null ? row.endDecile : ""}
                              onChange={e => updateSeg(idx, "endDecile", e.target.value === "" ? null : Number(e.target.value))}>
                              <option value="">—</option>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          <input type="number" min={0} max={52}
                            ...
                            style={{ width: 64, padding: "4px 6px", border: "1px solid #E2E0D8",
                              borderRadius: 4, fontFamily: "monospace", fontSize: 12,
                              textAlign: "center", outline: "none",
                              background: isLocked ? "#F5F4F1" : "#fff" }} />
                        </td>
                        <td style={{ padding: "8px 10px", textAlign: "center" }}>
                          <select style={{ ...styles.segSelectSm, width: 60 }}
                            disabled={isLocked}
