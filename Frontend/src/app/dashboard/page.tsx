...
export default function Dashboard() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (mapRes.status === "fulfilled") {
        // Extract stats from response headers to drive AI inference panel
        const statsHeader = mapRes.value.headers?.["x-stats-json"] || null;
        setResult({ mapHtml: mapRes.value.data, statsJson: statsHeader || "" });
        setTimeout(() => mapRef.current?.scrollIntoView({ behavior: "smooth" }), 300);
        // Trigger AI inference panel
        if (file) {
          setAiLoading(true);
          try {
            const aiFd = new FormData();
            aiFd.append("file", file);
            aiFd.append("num_clusters", String(numClusters));
            aiFd.append("column_config", JSON.stringify(cfg));
            aiFd.append("tolerance_pct", String(tolerance));
            const aiRes = await axios.post(`${API}/optimize_excel`, aiFd,
              { responseType: "blob", timeout: 300000 });
            // AI result already embedded in Excel — fetch inference separately
          ...
          } catch { /* AI panel is non-blocking */ }
  ...
  return (
    <>
      ...
      {/* Generate territory map overlay */}
      {generating && (
        <div className="overlay">
          <div className="overlay-card">
            <div className="overlay-spinner" />
            <div className="overlay-title">Generating territory map…</div>
            <div className="overlay-sub">Optimising clusters &amp; building Excel — this may take a minute</div>
          </div>
        </div>
      )}
