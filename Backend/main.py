...
# ---------------------------------------------------------------------------
def territory_alpha_shapes(df: pd.DataFrame, labels: np.ndarray,
                            weights: np.ndarray, k: int) -> dict:
    """
    Returns a dict {t_id: list of rings [[lat,lon],...]} using alpha shapes
    (concave hulls) so adjacent territory boundaries don't visually overlap.
    Falls back to convex hull if alphashape is unavailable.
    """
    ...
    for t_id in range(k):
        ...
        if len(nz_idx) < 3:
            continue
        pts_latlng = df.iloc[nz_idx][['latitude', 'longitude']].values
        pts_lonlat = [(float(lon), float(lat)) for lat, lon in pts_latlng]

        ring = None

        if HAS_ALPHASHAPE and len(pts_lonlat) >= 4:
            try:
                # Try progressively tighter alphas until we get a clean polygon
                for alpha in [0.5, 1.0, 2.0, 3.0, 5.0]:
                    candidate = alphashape.alphashape(pts_lonlat, alpha)
                    if (candidate and not candidate.is_empty
                            and candidate.geom_type in ('Polygon', 'MultiPolygon')):
                        geom = shapely_mapping(candidate)
                        if geom['type'] == 'Polygon':
                            ring = [[lat, lon] for lon, lat in geom['coordinates'][0]]
                        else:  # MultiPolygon — use largest sub-polygon
                            biggest = max(geom['coordinates'], key=lambda p: len(p[0]))
                            ring = [[lat, lon] for lon, lat in biggest[0]]
                        break
            except Exception:
                ring = None
        ...
        if ring is None:
            ...
            try:
                hull = ConvexHull(pts_latlng)
                ring = pts_latlng[hull.vertices].tolist()
                ring.append(ring[0])
            except Exception:
                pass
...
territory_convex_hulls = territory_alpha_shapes
...
def generate_map_html(df: pd.DataFrame, stats: dict, k: int, title: str) -> str:
    ...
    hulls = territory_alpha_shapes(df, labels_arr, weights_arr, k)
