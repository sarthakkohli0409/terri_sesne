...
def generate_map_html(df: pd.DataFrame, stats: dict, k: int, title: str) -> str:
    ...
    hulls = territory_shapes(df, labels_arr, weights_arr, k)
    hull_group = folium.FeatureGroup(name="Territory shapes", show=True)
    for t_id, hull_data in hulls.items():
        color = colors[t_id % len(colors)]
        s = stats.get(t_id, {})
        tooltip = f"T{t_id} | Index: {s.get('Weight')} | {s.get('Status')}"
        # hull_data can be a single ring or a list of rings (MultiPolygon)
        rings = hull_data if isinstance(hull_data[0][0], list) else [hull_data]
        for ring in rings:
            folium.Polygon(
                locations=ring,
                color=color,
