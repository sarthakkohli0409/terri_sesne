...
def run_scenario(df, k, min_cap, max_cap, hard_floor=650.0):
    ...
    # ── BORDER DONATION rebalance (no dissolve, no ghost territories) ──
    labels, cluster_loads = border_donation_rebalance(
        coords, weights, labels, cluster_loads,
        k, hard_floor, max_cap, max_rounds=20,
    )
...
def build_excel(df_res: pd.DataFrame, stats: dict, k_rec: dict,
    ...
    hdrs2 = ["Territory", "Status", "Index", "vs Target", "Active ZIPs", "Total ZIPs", "Diameter (mi)"]
    write_header_row(ws2, 2, hdrs2)
    col_widths2 = [14, 14, 12, 14, 14, 12, 16]
