...
# ---------------------------------------------------------------------------
def build_zip_neighbor_graph(coords: np.ndarray, n_neighbors: int = 8) -> dict:
    """
    For each ZIP index build a set of its n_neighbors nearest ZIP indices.
    Uses a simple distance matrix (cheap enough for typical dataset sizes).
    ...
    for start in range(0, n, chunk):
        ...
        dists = pairwise_distances(coords[start:end], coords)
        for local_i, row in enumerate(dists):
            global_i = start + local_i
            # Exclude self (distance 0), take n_neighbors nearest
...
def assign_zips_contiguous(
    ...
    dist_to_centroids = pairwise_distances(coords, centroids)
    seeded = set()
    for t_id in range(k):
...
def assign_zips(coords, weights, centroids, min_cap, max_cap, hard_floor=650.0):
    k = len(centroids)
    dist_matrix = pairwise_distances(coords, centroids)
...
# ---------------------------------------------------------------------------
def territory_convex_hulls(df: pd.DataFrame, labels: np.ndarray,
                            weights: np.ndarray, k: int) -> dict:
    """
    Returns a dict {t_id: [[lat,lon], ...]} of convex hull vertices
    for each territory that has >= 3 nonzero ZIPs.
    Uses a simple gift-wrapping convex hull on lat/lon points.
    """
    try:
...
def generate_territory_colors(k: int) -> list:
    """
    Generate k visually distinct colors using HSL rotation.
    Returns list of hex color strings.
    """
    colors = []
    for i in range(k):
        hue = int((i * 360 / k) % 360)
        # Convert HSL(hue, 65%, 55%) to hex
        h = hue / 360.0
        s, l = 0.65, 0.55
        if s == 0:
            r = g = b = l
...
# ---------------------------------------------------------------------------
# Map generation with convex hull fills
# ---------------------------------------------------------------------------
def generate_map_html(df: pd.DataFrame, stats: dict, k: int, title: str) -> str:
    ...
    colors = generate_territory_colors(k)
    status_colors = {"Green": "#1aa15a", "Yellow": "#d18a17",
                     "Red": "#d13b3b", "Orange": "#e07020"}

    active_df = df[df['final_weight'] > 0].copy()

    # ── Convex hull filled polygons (transparent) ──
    hulls = territory_convex_hulls(df, labels_arr, weights_arr, k)
    hull_group = folium.FeatureGroup(name="Territory shapes", show=True)
    for t_id, hull_pts in hulls.items():
        ...
        folium.Polygon(
            ...
            fill_color=color,
            fill_opacity=0.12,
            tooltip=f"T{t_id} | Index: {s.get('Weight')} | {s.get('Status')}",
        ).add_to(hull_group)
    hull_group.add_to(m)
    ...
    for t_id in range(k):
        ...
        for _, row in t_data.iterrows():
            features.append({
                ...
                "properties": {
                    "popup": f"T{t_id} | ZIP: {row['clean_zip']} | Index: {int(row['final_weight'])}",
                },
            })
        if features:
            folium.GeoJson(
...
def run_scenario(df, k, min_cap, max_cap, hard_floor=650.0):
    ...
    seeded_centroids = seed_centroids_bidirectional(coords, weights, k)
    ...
    # ── Build ZIP neighbor graph for contiguous assignment ──
    print(f"Building ZIP neighbor graph for {len(coords)} ZIPs...")
    zip_neighbors = build_zip_neighbor_graph(coords, n_neighbors=8)

    # ── Contiguous assignment (Prim-like BFS from centroids) ──
    print("Assigning ZIPs contiguously...")
    labels, cluster_loads = assign_zips_contiguous(
        coords, weights, centroids, min_cap, max_cap, hard_floor, zip_neighbors
    )

    # ── Contiguous rebalance (extreme-first, border-only donations) ──
    print("Rebalancing along borders...")
    labels, cluster_loads = contiguous_rebalance(
        coords, weights, labels, cluster_loads,
        k, hard_floor, min_cap, max_cap, zip_neighbors, max_rounds=30,
    )

    scenario_df['Territory_ID'] = labels

    # ── Compute centroids (lat/lon) for each territory ──
    centroids_latlon = {}
    for t_id in range(k):
        t_nz = np.where((labels == t_id) & (weights > 0))[0]
        if len(t_nz) > 0:
            centroids_latlon[t_id] = {
                "lat": round(float(coords[t_nz, 0].mean()), 5),
                "lon": round(float(coords[t_nz, 1].mean()), 5),
            }

    stats = compute_stats(
        scenario_df, labels, weights, k, min_cap, max_cap, hard_floor
    )

    # Attach centroid to each territory's stats
...
def generate_ai_inference(stats: dict, k: int, tolerance_pct: int,
    ...
    try:
        ...
        prompt = f"""You are a pharmaceutical territory optimization expert analyzing field force territory design.

Territory run summary:
- Total territories (K): {k}
- Index target: 1000 per territory (tolerance ±{tolerance_pct}%)
- Acceptable range: {min_cap}–{max_cap}
- Hard floor (minimum viable index): {hard_floor}
- Green (optimal): {len(green)} territories
- Yellow (slightly low): {len(yellow)} territories  
- Orange (slightly high): {len(orange)} territories
- Red (below hard floor): {len(red)} territories

Outlier territories needing attention:
{chr(10).join(outlier_detail) if outlier_detail else "None — all territories are optimal!"}

Respond ONLY with a JSON object in this exact structure, no preamble or markdown:
{{
  "executive_summary": "3-4 sentence executive narrative assessing overall territory quality and distribution balance.",
  "outlier_suggestions": [
    {{"territory": "T8", "issue": "brief issue description", "suggestion": "specific actionable fix"}}
  ],
  "strategic_recommendation": "One paragraph strategic recommendation for improving the overall territory design."
}}"""
...
async def optimize_excel(
    ...
    try:
        ...
        xlsx_bytes = build_excel(
            df_res, stats_res, k_rec, k, tolerance_pct,
