...
def generate_map_html(df: pd.DataFrame, stats: dict, k: int, title: str) -> str:
    ...
    # Build KD-tree to find 8 nearest neighbors per ZIP
    from scipy.spatial import cKDTree
    tree = cKDTree(coords)
    ...
    # ── ZIP dot markers — border ZIPs larger to outline territory ──
    dot_group = folium.FeatureGroup(name="ZIP markers", show=True)
    idx_map = {int(row['Territory_ID']): [] for _, row in active_df.iterrows()}
    for df_idx, row in active_df.iterrows():
        t_id = int(row['Territory_ID'])
        orig_idx = df.index.get_loc(df_idx) if df_idx in df.index else df_idx
        is_border = orig_idx in border_set
        idx_map[t_id].append((row, is_border))

    for t_id, zip_list in idx_map.items():
        color = colors[t_id % len(colors)]
        for row, is_border in zip_list:
            radius = 5 if is_border else 3
            opacity = 1.0 if is_border else 0.75
            folium.CircleMarker(
