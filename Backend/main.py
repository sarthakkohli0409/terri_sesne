...
def load_master_geodata(filename: str = "uszips.csv"):
    ...
    try:
        if os.path.exists(filepath):
            ...
            if required.issubset(MASTER_ZIP_DF.columns):
                ...
                if 'state_id' in MASTER_ZIP_DF.columns and 'state' not in MASTER_ZIP_DF.columns:
                    MASTER_ZIP_DF['state'] = MASTER_ZIP_DF['state_id']
                elif 'state' not in MASTER_ZIP_DF.columns:
                    MASTER_ZIP_DF['state'] = 'XX'
                print(f"Master geodata loaded: {len(MASTER_ZIP_DF)} rows, state col: {'state' in MASTER_ZIP_DF.columns}")
            else:
...
def state_preferring_rebalance(
    ...
    Phase 1 — donate border ZIPs preferring same-state neighbours first
    Phase 2 — relax to soft range for cross-state donations if needed
    """
    if soft_min is None:
        soft_min = hard_floor * 0.9
    if soft_max is None:
        soft_max = max_cap * 1.1
    ...
    def do_pass(donor_floor, recv_cap, same_state_only=False):
        ...
        for o_id in over:
            ...
            if prefer_state and same_state_only:
                o_state = states[np.where(labels == o_id)[0][0]] if (labels == o_id).any() else 'XX'
                adj = {t for t in adj if any(states[labels == t] == o_state)}
        ...
        for u_id in under:
            ...
            if prefer_state and same_state_only:
                u_state = states[np.where(labels == u_id)[0][0]] if (labels == u_id).any() else 'XX'
                adj = {t for t in adj if any(states[labels == t] == u_state)}
    ...
    # Phase 1: prefer same-state donations, hard range
    if prefer_state:
        print("Phase 1: state-preferring rebalance...")
        for rnd in range(max_rounds // 2):
            loads = get_loads()
            over  = [t for t in range(k) if loads[t] > max_cap]
            under = [t for t in range(k) if loads[t] < hard_floor]
            if not over and not under:
                print(f"  Phase 1 converged at round {rnd}")
                break
            if not do_pass(hard_floor * 0.5, max_cap, same_state_only=True):
                break

    # Phase 2: allow cross-state, soft range for remaining outliers
    print("Phase 2: soft-range cross-state rebalance...")
    for rnd in range(max_rounds):
        loads = get_loads()
        over  = [t for t in range(k) if loads[t] > max_cap]
        under = [t for t in range(k) if loads[t] < hard_floor]
        if not over and not under:
            print(f"  Phase 2 converged at round {rnd}")
            break
        if not do_pass(hard_floor * 0.5, soft_max, same_state_only=False):
            print(f"  Phase 2 stalled at round {rnd}")
            break
...
def run_scenario(df, k, min_cap, max_cap, hard_floor=650.0, prefer_state: bool = False):
    """
    ...
    # ── Step 3: State-preferring rebalance ─────────────────────────────────
    states_arr = scenario_df['state'].values if 'state' in scenario_df.columns else np.array(['XX']*len(scenario_df))
    labels, cluster_loads = state_preferring_rebalance(
        coords, weights, states_arr, labels, np.zeros(k),
        k, hard_floor, min_cap, max_cap, zip_neighbors,
        prefer_state=prefer_state,
        soft_min=hard_floor * 0.875,   # 750 when target=1000, floor=700
        soft_max=max_cap * 1.087,      # 1250 when max_cap=1150
        max_rounds=40,
    )

    # ── Legacy 3-pass fallback (kept for non-state mode) ─
    if False:
        pass
...
async def optimize_map(
    ...
    try:
        ...
        df_res, stats_res, k_rec = run_scenario(base_df, k, min_cap, max_cap, hard_floor, prefer_state=prefer_state)
        gc.collect()
